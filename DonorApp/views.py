...
def profile(request):
    ...
    for d in dd:
        ...
        table_data += "<td><a href=/donor/UpdateProfile?id=" + str(d[0]) + "&mobile=" + str(d[3]) + "&b_type=" + str(d[6]) + ">Update</a></td>"
        table_data += "<td><a href=/donor/DeleteProfile?id=" + str(d[0])+">Delete</a></td></tr>"
...
def DeleteProfile(request):
    ...
    con = Database.connect()
    cur = con.cursor()
    cur.execute("delete from donor where id='" + str(id) + "'")
    con.commit()
    return render(request, 'Donor/Login.html')
